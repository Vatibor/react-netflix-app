...
function PlansScreen() {
    ...
    const loadCheckout = async (priceId) => {
        ...
        docRef.onSnapshot(async(snap) => {
            ...
            if(error) {
                // show an error to customer and inspect your cloud function logs in the firebase console
                alert(`An error occured: ${error.message}`);             
            }

            if(sessionId) {
                 // we have a session, lets redirect to checkout, init stripe
                 const stripe =await loadStripe('pk_test_51JSJbUFZ4sC8rbs5OrXhQi0HOXd1xtF2qaLUkF9uLaJWzZFB9gV0UsjKJDt4dasq7Swmwe4f0B3de7ujfRKFmB9b00pI5cfGfq');
